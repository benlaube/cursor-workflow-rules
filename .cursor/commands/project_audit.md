...
**Source Checklist:** `docs/process/checklists/project_audit_checklist_v1_0.md`
...
- `deep`: `true` or `false` (default: `false`). If true, also run `rls_policy_review` and `security_audit` checks.
...
*If `deep=true`, also call `rls_policy_review`*
...
2. **RLS Policies:**
   ...
   - If `deep=true`, run full RLS policy review
...
1. Call `rls_policy_review` for detailed RLS analysis
...
- `security_audit` - Security-specific audit
