...
const createResult = await safeMutation(() => supabase.from('posts').insert({ title: 'New Post' }));
if (!createResult.ok) {
...
// Use in operations
const result = await interceptor.execute({ operation: 'select', resource: 'posts' }, () =>
...
const result = await safeSupabase.query(() => supabase.from('posts').select('*'));
